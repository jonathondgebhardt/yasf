...
auto main() -> int
{
    ...
    tree.add_node(std::make_unique<yasf::DecoratorNode>());

    auto manager = yasf::viewer::SceneManager{};
    auto behavior_tree_drawable = std::make_unique<BehaviorTreeDrawable>(&tree);
    manager.add_drawable(std::move(behavior_tree_drawable));
