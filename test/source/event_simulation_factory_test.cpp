...
TEST_CASE("event_simulation: queue", "[simulation]")
{
    struct concrete_processor : public yasf::EventProcessor
    {
        ...
        yasf::time::Microseconds m_event_time{};
        yasf::time::Microseconds m_clock_time{};
    };
    ...
    sim->queue(std::make_unique<yasf::Event>(
        yasf::convert::time_cast<yasf::time::Microseconds>(
            yasf::time::Seconds{3.0}),
        yasf::EventType::USER));
    sim->queue(std::make_unique<yasf::Event>(yasf::time::Microseconds{0},
                                             yasf::EventType::USER));
    sim->queue(std::make_unique<yasf::Event>(yasf::time::Microseconds{4},
                                             yasf::EventType::USER));
    sim->queue(std::make_unique<yasf::Event>(yasf::time::Microseconds{2},
                                             yasf::EventType::USER));
    ...
    CHECK(processor->m_event_time == yasf::time::Microseconds{0});
    CHECK(processor->m_clock_time == yasf::time::Microseconds{0});
    CHECK(sim->num_events() == std::size_t{3});

    REQUIRE_NOTHROW(sim->update());
    CHECK(processor->m_event_time == yasf::time::Microseconds{2});
    CHECK(processor->m_clock_time == yasf::time::Microseconds{2});
    CHECK(sim->num_events() == std::size_t{2});

    REQUIRE_NOTHROW(sim->update());
    CHECK(processor->m_event_time == yasf::time::Microseconds{4});
    CHECK(processor->m_clock_time == yasf::time::Microseconds{4});
