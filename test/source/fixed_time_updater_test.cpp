...
{
    constexpr auto delta_time_sec = yasf::time::Seconds{1.0};
    constexpr auto delta_time_usec =
        yasf::convert::seconds_to_useconds(delta_time_sec);

    SECTION("delta: ctor seconds")
    {
        auto const updater = yasf::FixedTimeUpdater{delta_time_sec};
        CHECK(updater.delta() == delta_time_usec);
    }

    SECTION("delta: ctor micro seconds")
    {
        auto const updater = yasf::FixedTimeUpdater{delta_time_usec};
        CHECK(updater.delta() == delta_time_usec);
    }

    SECTION("delta: setter seconds")
    {
        auto updater = yasf::FixedTimeUpdater{yasf::time::Seconds{0.0}};
...
{
    auto updater = yasf::FixedTimeUpdater{yasf::time::Seconds{1.0}};
